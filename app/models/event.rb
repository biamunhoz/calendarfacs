class Event < ApplicationRecord
    ...
    def ehfdsouferiado
      ...
      if @configsala.disablefds == true
        ...
        self.start_date.to_date.upto(self.end_date.to_date) do |day|
          ...
          if feriado(day)
            errors.add(:sala, "Evento não pode ser marcado, impossível marcar no feriado")
            break
          end
          ...
          when 0
            if self.domingo == true
              ...
              errors.add(:sala, "Evento não pode ser marcado, impossível marcar no fim de semana")
              break
            end 
          when 6
            if self.sabado == true
    ...
    def dia_is_checked
      errors.add(:Dia, "Evento não pode ser marcado. Por favor, selecione pelo menos um dia.") unless domingo || segunda || terca || quarta || quinta || sexta || sabado
    end
    ...
    def horariomarcado
      if validaFinal == true
        errors.add(:Data, "Evento não pode ser marcado. Período reservado anteriormente ou não permitido reserva")
      end  
    end
